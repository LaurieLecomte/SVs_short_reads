...
add_ALT <- function(input_vcf, output_vcf, refgenome = NULL) {
  ...
  starts <- vcf[[2]]
  altseq <- vcf[[5]]
  svlen  <- as.numeric(sub(".*SVLEN=(-?[0-9]+).*", "\\1", vcf[[8]])) # returns NA for SVs with ALT = <INS>
  ends <- as.numeric(sub("^END=(-?[0-9]+).*", "\\1", vcf[[8]]))      ######### addition by me, for manta the 8th field starts with END, need ^ because otherwise CIEND field will be grepped
  svtype <- sub(".*SVTYPE=([A-Z]+);.*", "\\1", vcf[[8]])
  
  widths <- ends - starts ######### addition by me # returns 0 for INSs. Used for DELs only
  ...
  del_info <- del_process(chrs[dels], starts[dels], widths[dels], refgenome = refgenome, len = svlen[dels])
  ins_info <- ins_process(chrs[ins],  starts[ins], refgenome = refgenome, len = svlen[ins], alt = altseq[ins])
  ...
  #vcf[[8]][dels] <- paste0('SVTYPE=', del_info$svtype, ';SVLEN=', del_info$svlen, ';END=', del_info$end, ';ALT_MANTA=', del_info$alt)
  
  vcf[[2]][ins]  <- ins_info$pos
  vcf[[4]][ins]  <- ins_info$ref
  vcf[[5]][ins]  <- ins_info$alt
  vcf[[8]][ins] <- paste0('SVTYPE=', ins_info$svtype, ';SVLEN=', ins_info$svlen, 
                          ';END=', ins_info$end, ';ALT_MANTA=', ifelse(ins_info$alt !='<INS>', ins_info$alt, '.'))
  ...
  #vcf[[8]][dups] <- paste0('SVTYPE=', dup_info$svtype, ';SVLEN=', dup_info$svlen, ';END=', dup_info$end, ';ALT_MANTA=', dup_info$alt)
  ...
  #vcf[[8]][invs] <- paste0('SVTYPE=', inv_info$svtype, ';SVLEN=', inv_info$svlen, ';END=', inv_info$end, ';ALT_MANTA=', inv_info$alt)
...
# DELs --------------------------------------------------------------------
del_process <- function(chr, start, width, refgenome, len) {
...
# INSs --------------------------------------------------------------------
ins_process <- function(chr, start, refgenome, len, alt) {
