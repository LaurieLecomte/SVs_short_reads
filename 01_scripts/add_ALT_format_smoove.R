...
add_ALT <- function(input_vcf, output_vcf, refgenome = NULL) {
  ...
  ends <- as.numeric(sub(".*;END=(-?[0-9]+).*", "\\1", vcf[[8]]))      ######### addition by me, for smoove END is preceded by ;, has to be added to regex because otherwise CIEND field will be grepped
  svtype <- sub(".*SVTYPE=([A-Z]+);.*", "\\1", vcf[[8]])
  ...
  #                        ';END=', ins_info$end, ';ALT_SMOOVE=', ifelse(ins_info$alt !='<INS>', ins_info$alt, '.'))
